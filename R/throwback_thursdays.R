...
tt_post <- function(date, title, url,
                    blurb = "<-- Interesting description/blurb mentioning the author -->",
                    dry_run = FALSE, print = FALSE) {

  m <- lubridate::month(date)

  if(length(date) > 1 & length(blurb) == 1) blurb <- rep(blurb, length(date))

  if(length(unique(lengths(list(date, title, url, blurb)))) > 1) {
    cli::cli_abort("{.arg date}, {.arg title} and {.arg url} all need to be the same length. If {.arg blurb} has more than one item, it must also match the lengths of the others.")
  }
  ...
  time_post <- date |>
    ...
    update(year = lubridate::year(Sys.Date()) + as.numeric(m < lubridate::month(Sys.Date()))) |>
