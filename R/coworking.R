...
cw_event <- function(date, dry_run = FALSE) {
  ...
  body <- glue::glue_data(
    ...
    "with cohost **{cohost}**, DESCRIPTION OF HOST.",
    "",
    ...
    "### Socialize/Network",
    "",
    "- Meet cohost, **{cohost}**, and discuss THEME.",
...
cw_slack_week <- function(x, posters_tz, dry_run = FALSE) {
  ...
  body <- x |>
    glue::glue_data(
      ...
      ":cookie: Hosted by: {who_main_slack} and cohost {who_slack}",
  ...
  body_sister <- x |>
    glue::glue_data(
      ...
      ":grey_exclamation: Theme: {theme}",
      ":hourglass_flowing_sand: When: {time}",
      ":cookie: Hosted by: @{who_main_linkedin} and cohost @{who_linkedin}",
      ":mag: Details: {event_url}",
      "",
      "You can use this time for...",
      "- General coworking",
